...
Button right;//오른쪽 화면으로 전화하는 버튼
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        proximity = manager.getDefaultSensor(Sensor.TYPE_PROXIMITY);//근접 센거의 값을 할당 합니다.
        if(proximity != null){
...
right.setOnClickListener(new View.OnClickListener() {
    ...
    public void start(View view) {
        ...
        condition = true;// 실행 상태를 true로 변경 합니다.
        tm = new timer();// 원형프로그레스바 사용을 위해 진행 쓰레드 객체를 생성 합니다.
        tm.start();//원형 프로그레스바 진행 시작 합니다.
        mCountDownTimer = new CountDownTimer(TIMER_DURATION, 1000) {// 카운트 다운 타이머 객체를생성 합니다.
            @Override
    ...
    public void onResumea(View view) {//정지된 타이머를 다시 재생할때 사용하는 메소드 입니다.
        condition = true;//타이머가 실행중이라는 것을 표시하고

        if (mCountDownTimer == null) { // 실행 중인 타이머가 없다면( 정지 기능을 구현하기 위해서 타이머를 삭제 하고 재시작할때 남은 시간으로 타이머를 다시 생성하여 타이머를 이어나갑니다.)
            mCountDownTimer = new CountDownTimer(mTimeRemaining, 1000L) {// 이전 타이머에서 남은 시간과 시간 간격을 입력하여 새로운 타이머르 만듭니다.
                @Override
                public void onTick(long millisUntilFinished) {//start()의 onTick과 같은 기능을 합니다.
                    time.setText(String.format(Locale.getDefault(), "%d: %d: %d", millisUntilFinished / 3600000L, millisUntilFinished%3600000L/60000L, millisUntilFinished%3600000L%60000L/1000L));
            ...
            }.start();//나머지 시간으로 만들어진 타이머 실행 합니다.
        }
    }
        ...
        if(condition==true||mTimeRemaining == 0) {//타이머가 실행 중이거나 또는 나머지시간이 없을때 즉 타이머가 실행되기전 초기화 상태일때
            if (pauseStatus == 0 && mTimeRemaining == 0) {//정지 횟수가 0이고 나머지 시간이 없을때 후식 횟수를 입력받습니다.
               ...
               final EditText number = (EditText) Pause.findViewById(R.id.npause);
                ...
                condition = false;//타이머를 정지 상태로 설정합니다.
...
tm.yield();//원형프로그레스 실행 쓰레드들 종료합니다.
    ...
    SensorEventListener listener=new SensorEventListener() {//센서 리스너를 생성합니다.
        ...
        public void onSensorChanged(SensorEvent sensorEvent) {//센서의 근사값이 바뀌면
            ...
            if(sensorEvent.sensor.getType()== Sensor.TYPE_PROXIMITY){//센서 이벤트로 근사값을 받았다면
                ...
                if(distance < proximityMaximumRange){//측정 거리가 1cm보다 작을때
                    if(startTime>pauseStatus&&mTimeRemaining==0) {//시작 횟수가 정지 횟수보다 많고 타이머가 실행되기 전에
        ...
        public void onAccuracyChanged(Sensor sensor, int accuracy) {//센서의 정확성도가 변경되었을때 실행됩니다. onSensorChanged 인터페이스를 구현하기위해 생성 되었습니다.
